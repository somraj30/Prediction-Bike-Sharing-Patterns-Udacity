...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        # Initialize weightsimport numpy as np
...
output_nodes = 1
        ...
        self.weights_input_to_hidden = np.random.normal(0.0, self.hidden_nodes**-0.5, # pXq size where p no. of nodes in input layer and 
                                       (self.input_nodes, self.hidden_nodes))        # q no. of nodes in hidden layer and r no. of output   
                                                                                     # nodes
        self.weights_hidden_to_output = np.random.normal(0.0, self.output_nodes**-0.5, # qXr size
                                       (self.hidden_nodes, self.output_nodes))
        self.lr = learning_rate
		...
		#self.sigmoid_prime = sigmoid_derivative
    ...
    def train(self, features, targets):
        ...
        '''
        n_records = features.shape[0]
        delta_weights_i_h = np.zeros(self.weights_input_to_hidden.shape) # pXq size
        delta_weights_h_o = np.zeros(self.weights_hidden_to_output.shape) # qXr size
        for X, y in zip(features, targets):
    ...
    def run(self, features):
        ...
        # TODO: Output layer - Replace these values with the appropriate calculations.
        final_inputs = final_inputs = hidden_outputs.dot(self.weights_hidden_to_output) # signals into final output layer
        final_outputs = final_inputs # signals from final output layer
